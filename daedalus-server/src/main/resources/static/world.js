...
(function (global) {
  ...
  const DRIVEN = {
    ...
    "parcel.revoke": {method: "POST", path: () => "/world/" + WORLD + "/parcels/revoke",
      body: (at, type, mazeId, actor) => ({x: at.x, y: at.y, z: at.z,
        actorId: actor || "alice",
        verb: type || "block.place"})},
    "parcel.forgive": {method: "POST", path: () => "/world/" + WORLD + "/parcels/forgive",
      body: (at, type, mazeId, actor) => ({x: at.x, y: at.y, z: at.z,
        actorId: actor || "alice",
        verb: type || "block.place"})},
  ...
  function paint(box, world, door, trap, portal, npc, chunk, trace) {
    ...
    events.forEach(frame => {
      ...
      const at = [frame.place, frame.lot, frame.box, frame.maze, frame.occupant,
          frame.drive, frame.driveActor, frame.driveAt].filter(Boolean).join(" ");
  ...
  function occupancy(obj) {
    ...
    const at = [obj.place, obj.lot, obj.box, obj.maze, obj.lease, obj.acl, obj.drive,
        obj.driveActor, obj.driveAt]
        .filter(Boolean).join(" ");
