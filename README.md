...
- **A vanilla web UI at `/`** — markup in `index.html`, paint in `draw.js`, HTTP in `api.js`, hash/walk helpers in `share.js`, leftover-state wiring in `app.js`, no npm, the same REST + STOMP
...
| `daedalus-server` | plugin-runtime, Spring Boot, Redis (optional) | Controllers (`MazeController`, `SessionController`, `LeaderboardController`, `InsightController`, `AgentController`, `CampaignController`, `MazeWebSocketController`, `PluginController`), DTOs in `com.daedalus.api.dto`, services (`MazeGenerationService` with Resilience4j circuit breaker, `LeaderboardService` with optional Redis backing, `GameSessionService`). Web UI at `src/main/resources/static/index.html` + `draw.js` + `api.js` + `share.js` + `app.js`. |
