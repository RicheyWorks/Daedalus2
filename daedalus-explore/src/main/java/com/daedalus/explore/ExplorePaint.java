...
public final class ExplorePaint {
    ...
    public static int[] brickTexColor(int x, int y) {
        ...
        int sx = x + ((y / 8) & 1) * 16;
        boolean course = y % 8 == 7;
        boolean joint = (sx % 16 == 1) || (sx % 16 == 15);
        if (course && joint) {
            return new int[] {
    ...
    static String placeOrBlock(ExploreBody body, ExploreMesh mesh, WorldMesh blocks) {
        ...
        if (last != null) {
            ...
            String boxes = lastParcelBoxes(blocks);
            ...
            if (boxes != null && !boxes.equals(box)) {
                named = named + " " + boxes;
            }
            if (maze != null) {
                named = named + " " + maze;
            }
            if (mazes != null && !mazes.equals(maze)) {
                named = named + " " + mazes;
            }
            if (rent != null) {
                named = named + " " + rent;
            }
            if (rents != null && !rents.equals(rent)) {
                named = named + " " + rents;
            }
    ...
    private static void blendCaptionEnd(float[] rgb, float brandR, float brandG, float brandB) {
        ...
        mixEndEdge(1, end);
        rgb[0] += (end[0] - rgb[0]) * FLOOR_END_WEIGHT;
        rgb[1] += (end[1] - rgb[1]) * FLOOR_END_WEIGHT;
        rgb[2] += (end[2] - rgb[2]) * FLOOR_END_WEIGHT;
    }
