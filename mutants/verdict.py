...
#: Any `SomethingTest.method` reference, used to attribute a failure to a class.
_TEST_METHOD = re.compile(r"\b(\w*Test)\.(\w+)")

#: Tests that compare recorded output against a stored copy rather than assert a property.
#:
#: A mutation whose *only* catcher is one of these has not been caught in any useful sense, and
#: `campaignteeth.py` is where that was measured. Four of its mutations were reported caught, and
#: with `DeterminismGoldenTest` excluded three of them survived: the campaign's target range could
#: revert to the configuration its own javadoc records as collapsing monotonicity from 60/60 seeds
#: to 34/60, the candidate pool could collapse to the single size recorded as 25/40, and the
#: hazard ramp could dump every hazard at the half-way mark. Every property test stayed green.
#:
#: A digest fires on *any* output change, which is why it looks like coverage — and it is
#: regenerated by whoever deliberately retunes a constant, which is exactly the moment the
#: regression it was accidentally covering walks through. So a digest-only failure is reported as
#: its own verdict and does not count toward the tally. Harnesses whose subject genuinely *is* the
#: snapshot machinery pass `digest_counts=True`; `detteeth.py` is the one that does.
SNAPSHOT_TESTS = frozenset({"DeterminismGoldenTest"})
...
def failing_classes(out):
    """Test *classes* named on Maven's `[ERROR]` lines — see `failing_tests` for why only those."""
    return {m.group(1) for line in out.splitlines() if line.startswith("[ERROR]")
            for m in _TEST_METHOD.finditer(line)}


def classify(returncode, out, failed=None, digest_counts=False):
    ...
    `digest_counts=True` opts out of the snapshot rule below — set it only when the harness is
    aimed at the snapshot machinery itself, where a golden test failing IS the property.
    """
    if returncode == 0:
        return "SURVIVED"
    ...
    classes = failing_classes(out)
    if not digest_counts and classes and classes <= SNAPSHOT_TESTS:
        return ("DIGEST ONLY -- the sole catcher is " + ", ".join(sorted(classes))
                + ", which notices that the bytes changed, not that a property broke")
    if have_names:
