...
**A golden test is not a catch, and `verdict.py` now enforces that.** `campaignteeth.py`
...
were accidentally covering walks through.

Doing that comparison by hand is not a rule, it is a habit that lasts as long as somebody
remembers, so the check moved into the module every harness already calls. When the only failing
class is in `verdict.SNAPSHOT_TESTS`, the verdict is `DIGEST ONLY` and it does **not** count
toward the tally — the harness reports it as unpinned and the author goes to find the property.
The fix for the campaign was twenty seeds instead of five in the monotonicity sweep and a hazard
assertion per rung instead of at the two ends; re-run under the new rule it still reads 12/12,
which is the point — every catch there now has a property behind it, and that is now a measured
statement rather than a remembered one.

One harness opts out: `detteeth.py` mutates the determinism check itself, so a golden failure
there *is* the property. It passes `digest_counts=True` and says why at the call site. If a second
snapshot-style test is ever added, add its name to `SNAPSHOT_TESTS` rather than teaching each
harness about it.
